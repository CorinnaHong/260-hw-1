...
```{r, make graphs , fig.width = 5, fig.height = 3}
...
adult_pair_scatter2 <- ggplot(adult_compare_tidy, aes(x = surgeonfish, y = spotted_damselfish)) +
  ...
  scale_color_manual(values=c("lightseagreen", "goldenrod2", "lightseagreen", "goldenrod2")) +
  geom_smooth(method = "lm", se = FALSE, color = "grey50", size = 0.5) +
  labs(x = expression(Surgeonfish~Density~("#"/100~m^2~of~reef)), y = expression(Spotted~Damselfish~Density~("#"/100~m^2~of~reef)), title = "Spotted Damselfish Dne v. Surgeonfish") +
  theme_minimal()

adult_pair_scatter2 # Finalize
